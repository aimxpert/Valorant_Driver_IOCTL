...
NTSTATUS WritePhysicalMemory(PVOID target_address, PVOID buffer, SIZE_T size, SIZE_T* bytes_read)
{
    ...
    custom_Memcpy(pmapped_mem, buffer, size);
...
NTSTATUS HandleReadRequest(pr Request) {
    ...
    if (Request->Write) {
    WritePhysicalMemory(PVOID(PhysicalAddress), (PVOID)((ULONG64)(Request->Buffer)), FinalSize, &BytesRead);
    }
    else
    {
